...
def get_pending_alerts_view(db: Session = Depends(get_db)):
    rows = db.execute(text("SELECT * FROM vw_pending_alerts")).fetchall()
    return [
        {
            "alert_id": row[0],
            "alert_date": str(row[1]),
            "alert_type": row[2],
            "status": row[3],
            "hours_pending": float(row[4]) if row[4] else 0.0,
            "seller_name": row[5],
            "client_name": row[6],
            "client_company": row[7]
        }
        for row in rows
    ]
''')
