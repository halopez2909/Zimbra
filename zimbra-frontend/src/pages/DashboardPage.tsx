    ...
    <div style={{ padding: "24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
        <div style={{ background: "#1F3864", borderRadius: "8px", padding: "20px", textAlign: "center" }}>
          ...
          <p style={{ margin: "8px 0 0", fontSize: "28px", fontWeight: "bold", color: "#fff" }}>{"$" + Number(totalAmount).toLocaleString()}</p>
        </div>
            ...
            <tbody>
              {traceability.map((t, i) => (
                <tr key={t.sale_id} style={{ background: i % 2 === 0 ? "#f9f9f9" : "#fff" }}>
                  ...
                  <td style={{ padding: "8px" }}>{"$" + Number(t.proposed_amount).toLocaleString()}</td>
                  <td style={{ padding: "8px", fontWeight: "bold", color: "#1E8449" }}>{"$" + Number(t.final_amount).toLocaleString()}</td>
